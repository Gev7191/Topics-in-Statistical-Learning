...
"Age55_Univ_M", "Age50_Univ_F", "Age55_NoDip_M", "Age55_NoDip_F","Age60_Sec_M","Age55_Sec_F", "Age55_Hi_M", "Age55_Hi_F", "Age60_Univ_M", "Age50_Univ_F", "Age55_NoDip_M", "Age55_NoDip_F","Age65_Sec_M","Age55_Sec_F", "Age55_Hi_M", "Age55_Hi_F", "Age65_Univ_M", "Age50_Univ_F", "Age55_NoDip_M", "Age55_NoDip_F")
...
RegressionPlots <- function(fit, textlabels, robust=F){
  ...
  # For scatter plot smoother
  LOESS <- loess.smooth(y, Fitted.Values)
  
  plt6 <- regMat %>% 
    plot_ly(x = y, y = Fitted.Values, 
            type = "scatter", mode = "markers", hoverinfo = "text",
            name = "Data", marker = list(size = 10, opacity = 0.5),
            text = paste('town: ', as.character(textlabels))) %>% 
    ...
    layout(title = paste(tt, " Fitted Values vs response"), plot_bgcolor = "#e6e6e6",
             xaxis = list(title = "Response values"),
             yaxis = list(title = "Fitted Values"))
...
# MM estimator
# 
mm = lmrob(y~., data = mod, intercept = TRUE, y=T, setting = "KS2014", control = lmrob.control(compute.rd = T))
summary(mm)
names(mm)
...
# 
# # LTS estimator
# lts = ltsReg(x, y, intercept = TRUE, alpha=0.9, y.ret=T)
# summary(lts)
# names(lts)
# plot(lts, cex = 0.6, id.n = 10) # , labels.id = newDat$town_name, sub.caption="LTS"
