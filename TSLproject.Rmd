...
#
# run classic MDS function on distances
selected_mds <- cmdscale(selected_mah, k = nrow(selected_mah) - 1,
         eig = TRUE)
...
#extreme_towns_m_f <- as.data.frame(cbind(firms$CODGEO, town))
#colnames(extreme_towns_m_f)[colnames(extreme_towns_m_f)=="V1"] <- "CODGEO"
...
### Moran's I statistic
```{r}
Moran <- cbind(geo[, c(2,7,8)], industry)
Moran <- aggregate(Moran[, 4:6], list(Moran$region), sum)

geo_coord <- geo[, c("region", "latitude", "longitude")]
geo_coord <- aggregate(geo_coord[, 2:3], list(geo_coord$region), mean)

distance_matrix <- dist(geo_coord[, c("latitude", "longitude")], method = "euclidean", diag = FALSE, upper = FALSE, p = 2)

Moran_distance_matrix <- as.data.frame(as.matrix(distance_matrix))

Moran_service <- Moran.I(Moran$nb_firms_service, Moran_distance_matrix, scaled = FALSE, na.rm = FALSE, alternative = "two.sided")

Moran_commerce <- Moran.I(Moran$nb_firms_commerce, Moran_distance_matrix, scaled = FALSE, na.rm = FALSE, alternative = "two.sided")

Moran_construction <- Moran.I(Moran$nb_firms_construction, Moran_distance_matrix, scaled = FALSE, na.rm = FALSE, alternative = "two.sided")


```


###Mapping the towns based on the intensity of locational Gini coefficient
```{r}
...
newDat_geo <-
  FraMap +
  geom_point(aes(x=geo$longitude, y=geo$latitude, colour= factor(kmeans(commune_pca_scores, 
                  centers = 6)$cluster)),
             data=geo, alpha=0.5, size=0.5) + labs(color='') + ggtitle("Clusters of towns based on 'commune' dataset predictors")
newDat_geo
```
...
# original scale
hist(newDat$sal_Males^-1, 30)
hist(newDat$sal_Females^-1, 30)
# log scale
hist(log10(newDat$sal_Males), 30)
hist(log10(newDat$sal_Females), 30)
# log scale: together
...
At the same time, for didactical reasons it is shown
...
From this study it is possible to conclude that each factor is relevant 
to the determine the salary level. 
The least important factor results the interaction between sex and job.
It means that it is not statistically significant the discrimination
of gender between various jobs, even if it exists in general and for
different ages.
...
The focus of this section is on the on the difference on salary between males and females in various towns.
...
x = cbind.data.frame(pop = newDat$total_population, 
                     ...
                     delta_transfer = newDat$transfer_M - newDat$transfer_F,
...
set.seed(5) # to ensure convergence
...
Model fitting using LTS estimator with trimming equal to 20% of the units: (does not work)
```{r LTS}

# lts_sol = ltsreg(y~., data = mod, alpha = 0.8)  # 
# lts_sol
# lts_sol$y = y
# plot(lts_sol)
# lts_sol$MD = covMcd(mod[,c(1:12, 14:ncol(mod))])$mah
# RegressionPlots(lts_sol, newDat$town, robust = T)

```

Specify the model with additional polynomials as predictors
```{r model augmentation}
...
Compute again OLS and MM solutions, which show that the benefit is marginal
```{r OLS and MM on augmented data}
...
# summary(mm)
# # outliers identified
# outl2 = mm$rweights < 2e-05
# # diagnostic plots
# RegressionPlots(mm, newDat$town, robust = T)

```

Use 10-folds cross validation on Elastic-net for different alpha values:
```{r CV for Elastic net}

# initialize list to contain the variables selected, 
# number of non zero coefficients and the counter
nzcoef = matrix(NA, 11, 2)
var_selec = list()
i = 1
...
# compare the coefficients obtained
print(var_selec)
...
The units' weights are now given to the same algorithm,
which were obtained using the MM estimator
```{r CV for Elastic net with MM weights}
...
for (j in alp){
  # set.seed (3)
  cv.out = cv.glmnet(x, y, alpha = j, nfolds=10, weights = mm$rweights) # standardize.response = T , parallel = T 
  plot(cv.out)
  title(paste("alpha = ", j), line = 0.3)
  # save selected variables
  tmp_coeffs <- coef(cv.out, s = "lambda.min") # cv.out$lambda.1se 
...
BSS results: (((DO ALL WITH A SUBSET)))
```{r BSS results}

# best subset selection
best.sub = regsubsets(y ~ ., data = mod, nvmax = ncol(mod), really.big=TRUE, method="seqrep") #exhaustive
...
summary(ols_bic)
RegressionPlots(ols_bic, textlabels = newDat$town)
...
# fit robust Lasso
sol_sparseLTS = sparseLTS(x, y, mode = "lambda", alpha = 0.95, ncores = NA, model=TRUE)
# check the solution
print(sol_sparseLTS) 
# construct mahalanobis distance (used for diagnostic plots)
sol_sparseLTS$MD = hatvalues(ols) # covMcd(x)$mah # compute standard leverages
...
```{r CV for Elastic net with sparseLTS weights}
...
for (j in alp){
  # set.seed (3)
  cv.out = cv.glmnet(x, y, alpha = j, nfolds=10, weights = sol_sparseLTS$wt) # standardize.response = T , parallel = T 
  plot(cv.out)
  title(paste("alpha = ", j), line = 0.3)
  # save selected variables
  tmp_coeffs <- coef(cv.out, s = "lambda.min") # cv.out$lambda.1se 
...
sol_enetLTS = enetLTS(x[,21:40], y, family="gaussian", alphas=seq(0, 1, 0.1), hsize=0.8, nfold=10, seed=1) # ,lambdas=0.05
# check the solution
print(sol_enetLTS)
...
RegressionPlots(sol_enetLTS, textlabels=newDat$town, robust = TRUE, out = as.factor(sol_enetLTS$wt))[[3]]
RegressionPlots(sol_enetLTS, textlabels=newDat$town, robust = TRUE, out = as.factor(sol_enetLTS$wt))[[4]]
# built-in plots
...
```{r CV for Elastic net with sol_enetLTS weights}
...
for (j in alp){
  # set.seed (3)
  cv.out = cv.glmnet(x, y, alpha = j, nfolds=10, weights = sol_enetLTS$wt) # standardize.response = T , parallel = T 
  plot(cv.out)
  title(paste("alpha = ", j), line = 0.3)
  # save selected variables
  tmp_coeffs <- coef(cv.out, s = "lambda.min") # cv.out$lambda.1se 
...
p <- plotly::plot_geo(newDat, lat = ~latitude, lon = ~longitude) %>%
  plotly::add_markers(
    text = ~paste(town, paste("Firms tot:", total), 
                  ...
                  sep = "<br />"),
    color = ~total_population,  size = I(3), hoverinfo = "text", opacity = 0.8 #, colors = 'Purples'
  ) %>%
