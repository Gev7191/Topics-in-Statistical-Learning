...
```{r}
firms_scaled <- scale(firms[, 3:8])
head(firms_scaled)

firms_truncated <- firms[, 4:8]
head(firms_truncated)
plot(firms)

# K-means clustering...

fitK_scaled <- kmeans(firms_scaled, 4) #scaling the data
fitK_scaled

fitK <- kmeans(firms_truncated, 5)
fitK
str(fitK)
plot(firms, col = fitK$cluster) #plotting data colored according to cluster membership
...
clusters
plot(firms, col = clusters) # as we can see, it performs quite similar to the K-means 
