...
```{r Pre-processing-Population data}
# preliminary checks
names(population)
summary(population)
...
```{r Create new variables and re-organize the dataset}

# Re-organize the data according to the CODGEO (no repetition over different categories of variables) and create new variables such as "child", "elderly" and "workforce" 
...
# Create new variables: dependent/ sex, dependency, aged and child dependency ratios
...
population$total_population_log <- log10(population$total_population)

# Histogram of total population per town
hist(population$total_population, ylim=c(0,40000), breaks = seq(0, 2500000, by=250000), xlab="", main = "", labels=T, col ="light blue")
# Histogram of total population per town
hist(population$total_population_log, xlab="Total Population in log", main = "Total population per town", col ="light blue")
# Histogram of dependency ratio per town
hist(population$dependency_ratio, xlab="", main = "dependency ratio", col ="light blue")
# Histogram of sex ratio per town
hist(population$sex_ratio, xlab="", main = "sex ratio", col ="light blue")
...
# FraMap
geo_population$total_population_log[geo_population$total_population_log == -Inf] = 0
sc <- scale_colour_gradientn(colours =palette(rainbow(3)), limits=c(min(geo_population$total_population_log), max(geo_population$total_population_log)))
population_distribution <-
  FraMap +
  geom_point(aes(x=geo_population$longitude, y=geo_population$latitude, colour=geo_population$total_population_log),
             data=geo_population, alpha=0.1, size=0.01) +
  sc +
  labs(color='Total Population in Log') +
  ggtitle("Distribution of Population for each CODGEO")
population_distribution

min(geo_population$total_population_log)
# ggmap(get_googlemap(center=fra_center, scale=2, zoom=5), extent="normal") +
#   geom_point(aes(x=lon, y=lat), data=geo_pos, col="orange", alpha=0.8, size=0.6) 
...
#myPalette(low = "white", high = c("green", "red"), mid=NULL, k =50)-Need "GLAD" package
sc <- scale_colour_gradientn(colours =palette(rainbow(2)), limits=c(min(geo_population$aged_dependency_ratio), max(geo_population$aged_dependency_ratio)))
...
# merging
population$CODGEO = as.integer(population$CODGEO)
newDat = merge(firmsNEW, population, by="CODGEO")
names(newDat)
head(newDat)
...
# Create new variables: total number immigrants per town generated by summation
# categ_socio$total_immig <- sum(categ_socio$M_immi_agri, categ_socio$F_immi_agri, categ_socio$M_immi_comm, categ_socio$F_immi_comm, categ_socio$M_immi_exec, categ_socio$F_immi_exec, categ_socio$M_immi_midman, categ_socio$F_immi_midman, categ_socio$M_immi_noAct, categ_socio$F_immi_noAct, categ_socio$M_immi_retired, categ_socio$F_immi_retired, categ_socio$M_immi_emp, categ_socio$F_immi_emp, categ_socio$M_immi_worker, categ_socio$F_immi_worker)
# 
# # total number of male immigrants per town 
# categ_socio$male_immig <- sum(categ_socio$M_immi_agri, categ_socio$M_immi_comm,categ_socio$M_immi_emp, categ_socio$M_immi_emp, categ_socio$M_immi_exec, categ_socio$M_immi_midman, categ_socio$M_immi_noAct, categ_socio$M_immi_retired, categ_socio$M_immi_worker)
# 
# # total number of female immigrants per town
# categ_socio$female_immig <- sum(categ_socio$F_immi_agri,categ_socio$F_immi_comm, categ_socio$F_immi_emp, categ_socio$F_immi_emp, categ_socio$F_immi_exec, categ_socio$F_immi_midman, categ_socio$F_immi_noAct, categ_socio$F_immi_retired, categ_socio$F_immi_worker)

# TRY FINDING A DIFFERENT WAY OF DOING THIS INSTEAD OF CREATING A NEW FILE AND MERGE THEM, JUST ADD THEM ON THE ORIGINAL FILE
# total number of immigrants and its number per gender, per town created
total_immigration <- ddply(categ_socio, .(CODGEO), function(categ_socio) {
...
# merge the total immigrants' number with categ_socio
categ_socio <- merge(categ_socio, total_immigration)
```
