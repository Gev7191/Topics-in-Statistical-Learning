...
geo <- subset(geo, select = -c(EU_circo, code_r?gion, num?ro_d?partement, pr?fecture, num?ro_circonscription, ?loignement))
# change names 
names(geo)[1:6] = 
  c("region", 
    "region_capital", 
...
# NO MORE NEEDED BECAUSE IS A CSV FILE
    ...
    # write.csv(sol, file = "geo_NA_Final.csv", row.names=FALSE)

# read the created csv
retrieved_geo_NA = read.csv("geo_NA_Final.csv", header = TRUE)

# get only long and lat and assign to original NA 
geo$latitude[indNA_coord] = retrieved_geo_NA[,3]
geo$longitude[indNA_coord] = retrieved_geo_NA[,2]
...
# to delete non-European countries
...
# gives error
# cv.err = cv.glm(salary, fit_LM_26_50, K = 3)$delta

summary(fit_LM)
plot(fit_LM)
```

Now we try to model salaries for younger people, which seems more difficult to predict:
```{r, fig.keep='all'}
...
population$SEXE <- factor(population$SEXE, levels = c(1,2), labels = c("Male", "Female"))
View(population)
...
uniq_cat <- unique(population$AGEQ80_17!=5)
uniq_cat
...
```{r}
# total ID in geo and not in pop
sum(unique(geo$CODGEO)) - sum(unique(population$CODGEO))
# total ID in pop that are and not in geo
sum(unique(population$CODGEO) %in% unique(geo$CODGEO) == 0)
# vice versa
sum(unique(geo$CODGEO) %in% unique(population$CODGEO) == 0)

# retain only complete units, i.e., losing ... obs
ind_pop = population$CODGEO %in% geo$CODGEO
population = population[ind_pop,]
ind_geo = geo$CODGEO %in% population$CODGEO 
geo = geo[ind_geo,]
```
