...
# creates common covariance matrix S
s <- 0
for (v in c(1:23)) S <- S + regions_count_sub$Freq[v] * as.matrix(firms_var[[v]]) 
S <- S / 5022
...
```{r}
# drops <- c("town", "total", "null", "total_population", "region", "region_capital", "department", "town_name", "postal_code", "latitude", "longitude", "LIBGEO.x", "LIBGEO.y", "REG", "DEP", "pop_2014", "transfer_M", "transfer_F", "M_immi_agri", "F_immi_agri", "male_immig", "tax_house14", "shared_tax_house14", "median_living14", "lev_ineq14")
# 
# 
...
# merge(newDat_variables, geo[1], by = "CODGEO", all = TRUE)
...
selected_pca_scores <- as.data.frame(scale(selected_pca_scores))
sapply(selected_pca_scores, var)
...
# K-means clustering
n <- nrow(selected_pca_scores)
wss <- rep(0, 15)
wss[1] <- (n - 1) * sum(sapply(selected_pca_scores, var))
for (i in 2:15)
    wss[i] <- sum(kmeans(selected_pca_scores,
                         centers = i)$withinss)
...
plot(selected_pca_scores, 
     pch = kmeans(selected_pca_scores, 
                  centers = 6)$cluster)
