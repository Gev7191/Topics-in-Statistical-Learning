...
geo_population[which.min(geo_population$aged_dependency_ratio),c('region')] 
geo_population[which.max(geo_population$aged_dependency_ratio),c('region')] 

# get list of top 10 highest aged dependency ratio
Rank = sort(geo_population$aged_dependency_ratio, TRUE)
Rank10 = sort(Rank[1:10], TRUE)
sol = rep(0, 10)
for (i in 1:10){
    sol[i] = paste(geo_population$region[geo_population$aged_dependency_ratio %in% Rank10[i]] )
}
...
The city with the lowest aged dependency ratio is ile-de-France and the highest is Pays de la Loire.
...
sol = rep(0, 10)
for (i in 1:10){
    sol[i] = paste(categ_socio$CODGEO[categ_socio$total_immig %in% Rank10[i]] )
}
...
= "75056" "13055" "69123" "31555" "6088"  "67482" "34172" "93066" "93001" "95018"
...
sol = rep(0, 10)
for (i in 1:10){
    sol[i] = paste(status_work$CODGEO[status_work$transfer %in% Rank10[i]] )
}
...
="75056" "13055" "6088"  "69123" "31555" "33063" "34172" "44109" "42218" "67482"

Paris has most individuals with half/full time contract.
Rouen has the least. 
= "75056" "69123" "13055" "31555" "44109" "59350" "67482" "34172" "33063" "35238"
...
sol = rep(0, 10)
for (i in 1:10){
    sol[i] = paste(status_work$CODGEO[ineq$unemp_rate %in% Rank10[i]] )
}
...
="33402" "30132" "59036" "76231" "97101" "2361"  "59291" "2304"  "8409"  "62447"
...
Load the reduced data and take a subsample of it to reduce the geo-spatial correlation
```{r}
...
salary_with_dep <- cbind(geo[1], salary)
#check
...
```{r}
# add region variable to the salary dataset
firms_with_dep <- cbind(geo[1], firms)
#check
#ncol(salary_with_dep)
#head(salary_with_dep)
# extract continuous variables
